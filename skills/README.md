...
[Claude Code](https://claude.com/claude-code) skills that turn ESM's raw graph into a guided, **structured-intent** practice. Each skill is a `SKILL.md` file with frontmatter (name, description, allowed ESM tools) and a body of operating instructions the assistant follows.
...
| [`session-protocol`](session-protocol/SKILL.md) | Operational protocol for *any* ESM session — how to load context, calibrate intent, capture signals richly, and keep the graph synchronized with the live conversation. Best loaded as standing context for every session. |
...
1. **Register the ESM MCP server.** The skills call `mcp__esm__*` tools, so the server must be registered as `esm` (see the repo [README](../README.md#mcp-connection)).
